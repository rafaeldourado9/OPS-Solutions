...
import {
  Plus, PencilSimple, Trash, Percent, CurrencyDollar, Tag,
  SpinnerGap, WarningCircle, X, Check,
} from '@phosphor-icons/react'
import { premisesApi, type Premise, type PremiseType, type CreatePremisePayload } from '../../api/premises'

function fmtValue(type: PremiseType, value: number): string {
...
function PremiseModal({ initial, onClose, onSave, saving }: ModalProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    onSave({ name: name.trim(), type, value: numVal, description: description.trim() })
  }
          ...
          <div>
            ...
            <div className="flex rounded-xl border border-zinc-200 overflow-hidden">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 text-[13px] font-semibold py-2 transition-colors ${
                  type === 'percentage' ? 'bg-[#0ABAB5] text-white' : 'bg-white text-zinc-500 hover:bg-zinc-50'
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 text-[13px] font-semibold py-2 border-l border-zinc-200 transition-colors ${
                  type === 'fixed' ? 'bg-[#0ABAB5] text-white' : 'bg-white text-zinc-500 hover:bg-zinc-50'
                }`}
              >
  ...
  return (
    <div className="bg-white rounded-2xl p-4 border border-zinc-100 shadow-sm flex items-start gap-4">
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
        premise.type === 'percentage' ? 'bg-[#0ABAB5]/10' : 'bg-violet-50'
      }`}>
        {premise.type === 'percentage'
      ...
      <div className="flex-1 min-w-0">
        ...
        {premise.description && (
          <p className="text-[12px] text-zinc-400 leading-snug mb-2.5">{premise.description}</p>
        )}
        <div className="flex items-center gap-2 flex-wrap mt-2">
          <span className={`text-[14px] font-bold font-mono ${
            premise.type === 'percentage' ? 'text-[#0ABAB5]' : 'text-violet-600'
          }`}>
            {fmtValue(premise.type, premise.value)}
          </span>
          <span className="text-[11px] px-2 py-0.5 rounded-full font-semibold bg-zinc-100 text-zinc-500">
            {premise.type === 'percentage' ? 'Percentual' : 'Valor fixo'}
          </span>
          {!premise.is_active && (
            <span className="text-[11px] px-2 py-0.5 rounded-full font-semibold bg-zinc-100 text-zinc-400">
              Inativa
            </span>
          )}
        </div>
...
export default function PremisesPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: CreatePremisePayload }) =>
      premisesApi.update(id, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['premises'] })
      setModal(null)
    },
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 pb-6">
      ...
      <p className="text-[13px] text-zinc-500 leading-relaxed">
        Configure os percentuais e valores que serão aplicados automaticamente nos orçamentos.
      </p>
      ...
      <div className="flex items-center gap-3 p-4 bg-[#0ABAB5]/8 rounded-2xl border border-[#0ABAB5]/20">
        <div className="w-9 h-9 rounded-xl bg-[#0ABAB5]/15 flex items-center justify-center shrink-0">
          <Tag size={16} weight="duotone" className="text-[#0ABAB5]" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-[13px] font-semibold text-[#1D1D1F]">
            Encargos percentuais totais:{' '}
            <span className="text-[#0ABAB5] font-mono">{totalPercent.toFixed(1).replace('.', ',')}%</span>
          </p>
          <p className="text-[11px] text-zinc-500 mt-0.5">Soma de todas as premissas percentuais ativas</p>
        </div>
        <div className="text-right shrink-0">
          <p className="text-[11px] font-semibold text-zinc-500">{premises.length} premissa{premises.length !== 1 ? 's' : ''}</p>
        </div>
      </div>
